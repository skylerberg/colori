...
fn resolve_pending_abilities_heuristic(state: &mut GameState, params: &HeuristicParams, rng: &mut impl Rng) {
    loop {
        ...
        match ability {
            ...
            Ability::MixColors { count } => {
                let (mixes, mix_count) = two_step_heuristic_mix_seq(
                    &state.players[player_index].color_wheel, count, &cache, params, rng,
                );
                for i in 0..mix_count {
                    let (a, b) = mixes[i];
                    perform_mix_unchecked(&mut state.players[player_index].color_wheel, a, b);
...
fn execute_solver_turn(state: &mut GameState, player_index: usize, heuristic_draft: bool, params: &HeuristicParams, rng: &mut impl Rng) {
    ...
    for i in 0..plan.action_count {
        ...
        match &action.action_type {
            SellPlanActionType::Workshop { selected_cards } => {
                ...
                resolve_pending_abilities_heuristic(state, params, rng);
            }
...
pub fn apply_solver_rollout_step<R: Rng>(state: &mut GameState, heuristic_draft: bool, params: &HeuristicParams, rng: &mut R) {
    ...
    match &state.phase {
        GamePhase::Action { action_state } => {
            let player_index = action_state.current_player_index;
            if action_state.ability_stack.is_empty() {
                execute_solver_turn(state, player_index, heuristic_draft, params, rng);
            } else {
