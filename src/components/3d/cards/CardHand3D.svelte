<script lang="ts">
  ...
  // Fan spread arc layout
  const ARC_RADIUS = 4.0;
  const MAX_TOTAL_ANGLE = Math.PI / 3; // 60 degrees
  ...
  function computeLayout(count: number): CardTransform[] {
    if (count === 0) return [];
    const totalAngle = Math.min(MAX_TOTAL_ANGLE, count * (Math.PI / 36));
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      const y = BASE_Y + ARC_RADIUS * (1 - Math.cos(angle)) * 0.3 + i * 0.001;
      const z = BASE_Z;
