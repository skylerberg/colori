...
use colori_core::opening_book::{OpeningBookParams, NUM_OPENING_BOOK_PARAMS};

fn run_opening_book_eval_game(
    book_params: &OpeningBookParams,
    eval_iterations: u32,
    glass: bool,
    rng: &mut WyRand,
) -> (f64, f64) {
    ...
    let book_config = MctsConfig {
        iterations: eval_iterations,
        opening_book_params: Some(Box::new(book_params.clone())),
        ..MctsConfig::default()
    };
    ...
    // Alternate which player uses the opening book (caller handles position alternation)
    let configs = [book_config, baseline_config];
...
pub fn run_opening_book_optimization(args: &SimulationArgs, ga: &CmaEsArgs) {
    let batch_id = generate_batch_id();

    eprintln!(
        "Opening Book CMA-ES: lambda={}, generations={}, games_per_eval={}, eval_iterations={}, initial_sigma={}, threads={}",
        ga.population, ga.generations, ga.games_per_eval, ga.eval_iterations, ga.initial_sigma, args.threads
    );
    ...
    let seed_genes = vec![0.0f64; NUM_OPENING_BOOK_PARAMS];
    let frozen_genes = colori_core::opening_book::frozen_gene_indices();
    ...
    for gen in 0..ga.generations {
        ...
        let eval_params: Vec<OpeningBookParams> = offspring
            .iter()
            .map(|g| OpeningBookParams::from_vec(g))
            .collect();
            ...
            std::thread::scope(|s| {
                ...
                for t in 0..num_threads {
                    ...
                    handles.push(s.spawn(move || {
                        ...
                        for game_idx in 0..count {
                            if game_idx % 2 == 0 {
                                let (w, _) = run_opening_book_eval_game(params, eval_iterations, glass, &mut rng);
                                thread_wins += w;
                            } else {
                                // Swap positions: book plays as player 1
                                let (_, w) = run_opening_book_eval_game(params, eval_iterations, glass, &mut rng);
                                thread_wins += w;
                            }
                        }
        ...
        let best_idx = fitness[0].0;
        let best_fitness = fitness[0].1;
        let best_params = OpeningBookParams::from_vec(&offspring[best_idx]);

        let output_path = format!("{}/opening-book-{}-gen-{}.json", args.output, batch_id, gen);
        let json = serde_json::to_string_pretty(&best_params).unwrap();
        std::fs::write(&output_path, json).unwrap();
    ...
    eprintln!("Opening book CMA-ES optimization complete. Results in {}/", args.output);
}
